...
const getProtectedResource = resourceId => __.find(protectedResources, protectedResource => protectedResource.resource_id === resourceId);

const getUser = username => userInfo[username];
...
app.get("/authorize", (req, res) => {
    ...
    let rscope = null, cscope = null, urlParsed = null;
    if (!client) {
        ...
        if (__.difference(rscope, cscope).length > 0) {
            urlParsed = buildUrl(req.query.redirect_uri, {
                error: 'invalid_scope'
            });
...
app.post('/introspect', (req, res) => {
    ...
    if (resourceSecret !== resource.resource_secret) {
        res.status(401).end();
        return;
    }
